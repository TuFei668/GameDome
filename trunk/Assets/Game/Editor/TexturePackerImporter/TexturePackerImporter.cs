...
public class TexturePackerImporter : Editor {
    ...
    protected static void CreateAtlas()
    {
        ...
        string inputPath = string.Format("{0}/_UI/Texture", Application.dataPath);//小图目录
        ...
        foreach(string path in paths)
        {
            ...
            string sheetPath = string.Format("{0}/_UI/Atlas/{1}", Application.dataPath, name);//用TP打包好的图集存放目录
            Helper.RunCmd("E:/TexturePacker/TexturePacker.exe", string.Format(args, sheetPath, sheetPath, sb.ToString()), "");
    ...
    {
        string atlasPath = string.Format("{0}/_UI/Atlas", Application.dataPath);
